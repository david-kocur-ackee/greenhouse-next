...
import type { DayPick } from 'src/domain/DayPick';
import type { WeekDay } from 'src/domain/WeekDay';
import durationToString from 'src/utils/durationToString';

import HourPicker from '~modules/UI/HourPicker/HourPicker';
import MiniDayPicker from '~modules/UI/MiniDayPicker/MiniDayPicker';
...
export default function CreateIntervalModal({ open, onClose, onAdd }: IntervalModalProps) {
    ...
    const [dayPicks, setDayPicks] = useState<DayPick[]>(() => weekDays.map(day => ({ day, picked: false })));

    const updateDayPicks = (day: WeekDay, value: boolean) => {
        setDayPicks(prev => prev.map(dp => (dp.day === day ? { ...dp, picked: value } : dp)));
    };
    ...
    const generateNewIntervals = () => {
        const newIntervals: CreateIntervalDto[] = [];
