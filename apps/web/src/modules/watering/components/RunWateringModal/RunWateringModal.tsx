...
import DurationPicker from '~modules/UI/DurationPicker/DurationPicker';
import Modal from '~modules/UI/Modal/Modal';

import useGetToggleResponse from '../WateringPage/hooks/useGetToggleResponse';
...
export default function RunWateringModal({ open, onClose, onRun }: RunWateringModalProps) {
    const [duration, setDuration] = useState(5);

    const getToggleResponse = useGetToggleResponse();
        ...
        <Modal title='Start watering system ' open={open} onClose={onClose}>
            <div data-testid='watering-modal'>
                ...
                <div className='px-4 py-3 flex flex-row-reverse sm:px-6'>
                    <button
                        type='button'
                        disabled={!isValid(duration) || !open || getToggleResponse.data?.state}
