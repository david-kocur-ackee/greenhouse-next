...
export default function UpdateIntervalModal({ open, toUpdate, onClose, onUpdate }: IntervalModalProps) {
    useEffect(() => {
        setStartTime(
            DateTime.fromObject({
        ...
        <Modal title='Update interval' open={open} onClose={onClose}>
                ...
                <div className='flex items-center justify-center sm:items-start'>
                    <div className='mt-3 flex flex-col text-center sm:ml-4 sm:mt-0'>
                        ...
                        <div className='text-xl'>
                            Total time:{' '}
                            <span className='font-semibold'>{isValid() ? durationToString(duration) : 'Invalid'}</span>
                        </div>
                    </div>
                </div>
            </div>
            <div className='px-4 py-3 flex flex-row-reverse sm:px-6'>
                <button
                    ...
                    onClick={() => {
                        onUpdate(new IntervalDto(toUpdate.id, startTime, endTime, toUpdate.weekDayIndex));
                        onClose();
                    }}
                >
                    Confirm
